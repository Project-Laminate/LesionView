classdef CvsView_exported < matlab.apps.AppBase
    ...
    properties (Access = private)
        ...
        cvsNiftiName
        backupLesion1 % Raw lesion mask in original space (never resampled)
        rawLesionInfo % NIfTI header info for raw lesion mask (for exact export matching)
        OverlayAlpha = 1;
    ...
    methods (Access = private)
        ...
        function analyzeLesions(app)
            ...
            % --- Simple Labeling in RAW Space ---
            % Label the raw lesion mask directly (backupLesion1 is raw original mask)
            updateProgress(app, 'Labeling lesions in raw space...');

            bwRaw = app.backupLesion1 > 0.8;
            app.L1raw = uint32(bwlabeln(bwRaw, 26));

            N = double(max(app.L1raw(:)));
            if isempty(N) || N==0, N = 1; end

            % --- Calculate stats in RAW space ---
            vi = double(app.L1raw(:));
            vi = vi(vi>=1 & vi<=N);
            ...
            if minVox>0
                ...
                if ~isempty(kill)
                    ...
                    N = double(max(app.L1raw(:)));
                    if N > 0
                        cnt = accumarray(double(app.L1raw(app.L1raw>0)),1,[max(N,1),1]);
                        cnt = cnt(1:N);
                    else
                        cnt = [];
                    end
            ...
            % ---- Sort so label 1 is LARGEST (by RAW volume) and relabel ----
            labels = find(cnt>0);
            if ~isempty(labels)
                [~,ord] = sort(cnt(labels),'descend');
                ranked = labels(ord);
                lut = zeros(max(labels),1,'uint32');
                lut(ranked) = uint32(1:numel(ranked));

                % Apply relabeling
                fg = app.L1raw>0;
                app.L1raw(fg) = lut(double(app.L1raw(fg)));
            ...
            % --- Generate View Space Labels (Replicate Loading Transform) ---
            updateProgress(app, 'Generating view space labels...');

            % 1. Calculate Resampling Parameters
            if isempty(app.FlairInfo) || isempty(app.rawLesionInfo)
                % Fallback if infos missing (shouldn't happen if loaded correctly)
                updateProgress(app, 'Warning: Missing header info, using backup resize');
                app.L1 = imresize3(app.L1raw, size(app.Lesion1), 'nearest');
            else
                originalPxDimFlair = app.FlairInfo.PixelDimensions;
                originalPxDimLesion = app.rawLesionInfo.PixelDimensions;
                refSpacing = min(min(originalPxDimFlair), min(originalPxDimLesion));

                scaleFactorsLesion = originalPxDimLesion / refSpacing;
                scaleFactorsFlair = originalPxDimFlair / refSpacing;

                newSizeLesion = round(size(app.backupLesion1) .* scaleFactorsLesion);
                newSizeFlair = round(app.FlairInfo.ImageSize .* scaleFactorsFlair);

                % 2. Resample to intermediate isotropic space
                L1_view = imresize3(double(app.L1raw), newSizeLesion, 'nearest');

                % 3. Apply Padding (match logic in LoadBIDS)
                % Note: app.FlairStar is already padded.
                % We use the logic: desired_z = size(app.FlairStar, 1) (which is X dim)
                % This assumes the Loading step enforced cubic-like Z dim.
                % If app.FlairStar is available, use its dimensions.

                if ~isempty(app.FlairStar)
                    desired_z_size = size(app.FlairStar, 1); % Logic from LoadBIDS
                    current_z_size = newSizeFlair(3);
                    ...
                    if current_z_size < desired_z_size
                        ...
                        % Pad Z dimension
                        L1_view = cat(3, zeros(size(L1_view,1), size(L1_view,2), pad_before), ...
                            L1_view, ...
                            zeros(size(L1_view,1), size(L1_view,2), pad_after));
                    end
                end

                app.L1 = uint32(L1_view);
            end

            % Ensure app.L1 matches app.Lesion1 size (sanity check)
            if ~isempty(app.Lesion1) && ~isequal(size(app.L1), size(app.Lesion1))
                % If mismatch (e.g. due to slightly different logic or rounding), resize to match
                updateProgress(app, 'Adjusting view label size to match image...');
                app.L1 = imresize3(app.L1, size(app.Lesion1), 'nearest');
            end

            updateProgress(app,'Calculating lesion centers ...')

            % Calculate the center of each lesion for visualization
            % CRITICAL: Derive index from L1raw to ensure all labels are accounted for in export
            app.lesionIndex = nonzeros(unique(app.L1raw));

            % Default centers to middle of image (for invisible lesions)
            centerDefault = round(size(app.L1)/2);
            if isempty(centerDefault), centerDefault = [1 1 1]; end
            app.lesionCenter = repmat(centerDefault, numel(app.lesionIndex), 1);
            ...
            for ii = 1:numel(app.lesionIndex)
                idx = app.lesionIndex(ii);

                % Store raw voxel count in column 2
                if idx <= numel(app.lesionVol)
                    app.sizeChange(ii, 2) = app.lesionVol(idx);
                end

                % Check if lesion exists in View Space
                if ismember(idx, tmpL1)
                    ...
                    [~, app.lesionCenter(idx, 1)] = max(squeeze(sum(sum(binaryMat, 2), 3)));
                    [~, app.lesionCenter(idx, 2)] = max(squeeze(sum(sum(binaryMat, 1), 3)));
                    [~, app.lesionCenter(idx, 3)] = max(squeeze(sum(sum(binaryMat, 1), 2)));
                else
                    % Use Raw Volume if invisible in View
                    if ii <= numel(app.lesionVol)
                        app.sizeChange(ii, 1) = app.lesionVol(ii);
                    end
                end
            end
   ...
   % Open folder selection dialog
    ...
    try
        ...
        if ~isempty(lesion1File)
            ...
            % Store raw lesion mask header info for exact export matching
            app.rawLesionInfo = niftiinfo(LesionPath);
    ...
    close all;
        ...
        function ExportNIfTIButtonPushed(app, event)
            ...
            try
                ...
                % CRITICAL: Use raw lesion mask info for EXACT space matching
                if isempty(app.rawLesionInfo)
                    ...
                    if ~isempty(lesionFile)
                        app.rawLesionInfo = niftiinfo(fullfile(lesionFile(1).folder, lesionFile(1).name));
                    else
                        error('Cannot find raw lesion mask info for export');
                    end
                end

                % Verify L1raw matches raw lesion mask size exactly
                if ~isequal(size(app.L1raw), size(app.backupLesion1))
                    error('L1raw size (%s) does not match raw lesion mask size (%s)', ...
                        mat2str(size(app.L1raw)), mat2str(size(app.backupLesion1)));
                end
                ...
                % Use raw lesion mask header, ensure exact size match
                info = app.rawLesionInfo;
                info.ImageSize = size(app.cvsMat); % Should already match, but ensure it
                info.Datatype = 'single';
                info.BitsPerPixel = 32;
                
                % Verify size matches exactly
                if ~isequal(info.ImageSize, size(app.cvsMat))
                    error('CVS map size mismatch with raw lesion mask header');
                end
                
                % write CVS probability map
                niftiwrite(app.cvsMat, app.cvsNiftiName, info, 'Compressed', true);
                updateProgress(app,'CVS NIfTI exported!');
                ...
                updateProgress(app, ['Export error: ' ME.message]);
                rethrow(ME);
            end
        end
